...
const IndividualNotificationPanel = () => {
  ...
  const loadOffers = async () => {
    try {
      ...
      setOffers(activeOffers.map(o => ({
        ...
        is_recurrent: o.is_recurrent || false,
        usage_limit: o.usage_limit || null
      })));
    } catch (err) {
  ...
  const fetchOffersRedemptions = async () => {
    try {
      ...
      setRedemptions(activeRedData.map(r => ({
        offer_id: r.offer_id,
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter(
      (m) =>
        ...
        ((m.category_name || m.customer_categories?.name) &&
          (m.category_name || m.customer_categories?.name)!.toLowerCase().includes(query))
    );
  }, [members, searchQuery]);
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge variant="outline">
                          {member.category_name || member.customer_categories?.name || "Uncategorized"}
                        </Badge>
                      </TableCell>
                    </TableRow>
                  );
